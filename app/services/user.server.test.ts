...
describe("UserServer", () => {
  describe("getUserById", () => {
    ...
    test("should throw if not a user id format", async () => {
      await expect(() =>
        UserServer.getUserById("not-user-123")
      ).rejects.toThrow();
    });
  });
  ...
  describe("getUserByEmail", () => {
    ...
    test("should throw if not an email", async () => {
      await expect(() =>
        UserServer.getUserByEmail("not-an-email")
      ).rejects.toThrow();
    });
  });
  ...
  describe("createUser", () => {
    ...
    test("should throw if user with email already exists", async () => {
      ...
      await expect(() =>
        UserServer.createUser("foo@bar.com", "1234abcd", "Guest", false, "foo")
      ).rejects.toThrow(ConflictError);
    });

    test("should throw if not a valid password", async () => {
      await expect(() =>
        UserServer.createUser("foo@bar.com", "short", "Guest", false, "foo")
      ).rejects.toThrow();
    });

    test("should throw if not an email", async () => {
      await expect(() =>
        UserServer.createUser("not-an-email", "1234abcd", "Guest", false, "foo")
      ).rejects.toThrow();
    });
  });
  ...
  describe("verifyLogin", () => {
    ...
    test("should throw if invalid password", async () => {
      ...
      await expect(() =>
        UserServer.verifyLogin("foo@bar.com", "incorrect-pw")
